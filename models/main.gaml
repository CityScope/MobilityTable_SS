...
global {
	...
	list wanderCountCar_plot <- list_with(8652, 0);
	list getFuelCount_plot <- list_with(8652, 0);
	// UDP connection
	/*int port <- 9877;
	...
	reflex updateValues {
		...
		if (cycle > 8652){
			...
			remove first(wanderCountCar_plot) from: wanderCountCar_plot;
			remove first(getFuelCount_plot) from:  getFuelCount_plot;
		}
		add avgWait to: avgWait_plot;
		add current_date.hour to: time_plot;
		add 40 to: limitwait_plot;
		add wanderCountCar to: wanderCountCar_plot;
		add  getFuelCount to:  getFuelCount_plot;
		/*if (cycle < 8652) {	
			add avgWait to: avgWait_plot;
			add current_date.hour to: time_plot;
			add 40 to: limitwait_plot;
...
experiment test type: gui benchmark: false keep_simulations: false {
	int fontSize <- 5;
	int x_val <- 100;
	int x_step <- 300;
	int y_val <- 3000;
    ...
    output {
		...
		display dashboard  antialias: false type: java2D background: #black{ 
			chart "Average Wait Time" type: series background: #black title_font: font("Helvetica", 15, #bold) title_visible: false color: #white axes: #white x_range: 8652 y_range:[0,120] tick_line_color:#transparent x_label: "" y_label: "" x_serie_labels: time_plot x_tick_unit: 721  memorize:false position: {100,800} size:{2450,600} series_label_position: none {
				data "Wait Time" value: avgWait_plot color: #pink marker: false style: line;
				//data "40 min" value: limitwait_plot color: #red marker: false style: line;
			}
			
			chart "Vehicle Tasks" type: series  background: #black color: #white title_font: font("Helvetica", 15, #bold) title_visible: false axes: #white tick_line_color:#transparent x_range:8652 x_label: "" y_label: "" x_serie_labels: time_plot  x_tick_unit: 721 memorize:false position: {100,1525} size:{3000,600} series_label_position: none{
    			
    			//data "Idling Cars" value: wanderCountCar_plot color: #dimgray marker: false style: line ;
				//**data "cars low battery/fuel" value: lowFuelCount color: #orange marker: false style: line;
				data "Recharging/Refuelling" value: getFuelCount_plot color: #red marker: false style: line ;
				//data "Cars in Use" value: inUseCountCar+pickUpCountCar color: #cyan marker: false style: line;
